...
bot.on("message",async message =>{
    ...
    if(cmd === `${prefix}yt` && message.channel.name === "music" && message.channel.type === "text"){
            ...
            ytsr.getFilters(arg, function(err, filters) {
                ...
                ytsr.getFilters(filter.ref, function(err, filters) {
                    ...
                    ytsr(null, options, function(err, searchResults) {
                        ...
                        /*
                        Due to lack of goto statements if not my lack of knowledge about them
                        I made this "not so efficient implimentation" by making a seperate if
                        for play function away from the if else if ladder and the following code 
                        will change the arg to a I'm feeling lucky URL for a youtube video and
                        change the cmds to "play" so that it enters the following if statement.
                        Till i find a better way i think this is the way it's going down.
                        */
                        arg = searchResults.items[0].link;
                        cmds = "play";
                        sflag = true;
                    });
                });
        ...
        if(cmds === "play"){
            //Play Command
            //Checking whether valid URL exists for the user input else skipping for splay generated URL
            if(!sflag){
                if(!arg){
